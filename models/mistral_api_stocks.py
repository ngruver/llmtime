...
def mistral_api_stocks_tokenize_fn(str, model):
    """
...
def mistral_api_stocks_completion_fn(model, input_str, steps, settings, num_samples, temp):
    """
    ...
    """
    avg_tokens_per_step = len(mistral_api_stocks_tokenize_fn(input_str, model)) / len(input_str.split(settings.time_sep))
    ...
    if model in ['mistral-tiny','mistral-small','mistral-medium']:
        mistral_sys_message = "You are a trading expert that performs time series predictions on the SPY index. The user will provide a sequence and you will predict the remaining sequence taking in consideration your expertise about the SPY stock index, do not hallucinate. The sequence is represented by decimal strings separated by commas."
...
def mistral_stocks_api_nll_fn(model, input_arr, target_arr, settings:SerializerSettings, transform, count_seps=True, temp=1):
    """
    ...
    predicted_arr = np.array(predicted_str.split(settings.time_sep)[:-1], dtype=float)
    predicted_arr = vmap(transform)(predicted_arr)
    predicted_str = serialize_arr(predicted_arr, settings)
    predicted_tokens = mistral_api_stocks_tokenize_fn(predicted_str, model)
    target_tokens = get_allowed_ids(target_str.split(settings.time_sep), model)
    nll = -np.sum(np.log(np.clip(predicted_tokens, a_min=1e-10, a_max=None)[target_tokens])) / len(target_tokens)
    return nll*10000000
    """""
    return -1
