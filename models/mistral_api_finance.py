...
class MistralApiFinance():


    def __init__(self):
        self.loaded_model = ''
        self.mistral_client = None
    def init_mistral_client(self,model):
        """
        Initialize the Mistral client for a specific LLM model.
        """
        if self.mistral_client == None or self.loaded_model != model:
            self.loaded_model = model
            self.mistral_client = MistralClient(os.environ['MISTRAL_KEY'])
    ...
    def mistral_api_completion_fn(self,model, input_str, steps, settings, num_samples, temp):
        """²
    ...
    def mistral_api_nll_fn(self,model, input_arr, target_arr, settings:SerializerSettings, transform, count_seps=True, temp=1):
        """
